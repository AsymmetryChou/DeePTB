...
class NanoTCADInputSet(object):
    ...
    def hamiltonian_get(self,allbonds:torch.tensor,hamil_block:torch.tensor,overlap_block:torch.tensor,Hamil_sisl:sisl.hamiltonian,energy_unit_option:str):
        ...
        print(len(allbonds))
        print(allbonds[0])
        np.save(self.results_path+'allbonds.npy',allbonds.detach().numpy())

        # H_device.H[1000,1000]=1
        num_orb =4
        Hamil_tcad = np.zeros((len(allbonds)+1,2+num_orb*num_orb))
        Hamil_tcad[0][0] = num_orb

        for i in range(len(allbonds)):
            if i%100==0:print('bond_index: ',i)
            if not allbonds[i][-3:].equal(torch.tensor([0,0,0])): print("attention:",allbonds[i])
            if allbonds[i][-3:].equal(torch.tensor([0,0,0])): #allbonds[i,1] is atom index             
                assert num_orb == hamil_block[i].detach().numpy().shape[0] and num_orb == hamil_block[i].detach().numpy().shape[1]
                i_atom = allbonds[i].detach().numpy()[1].astype(np.int32)+1
                j_atom = allbonds[i].detach().numpy()[3].astype(np.int32)+1
                
                Hamil_tcad[i+1][0] = i_atom
                Hamil_tcad[i+1][1] = j_atom
                Hamil_tcad[i+1][2:] = hamil_block[i].detach().numpy().ravel()*unit_constant
        

        nonzero_rows = np.any(Hamil_tcad != 0, axis=1)
        Hamil_tcad = Hamil_tcad[nonzero_rows]

        np.save(self.results_path+'Hamil_tcad.npy',Hamil_tcad)


    def cal_leads_selfenergy(self):

        
        self.negf_hamiltonian = NEGFHamiltonianInit(apiH=self.apiHrk, structase=self.structase, \
                                                    stru_options=self.jdata["stru_options"], results_path=self.results_path)
        with torch.no_grad():
            _, struct_leads = self.negf_hamiltonian.initialize(kpoints=self.kpoints)


        self.lead_L=LeadProperty(
        ...
        Elower = self.jdata['emin'] + self.jdata['e_fermi']
        Eupper = self.jdata['emax'] + self.jdata['e_fermi']
        dE = self.jdata['espacing']; eta_lead = self.jdata['eta_lead']
        E = Elower
        Erange = []
        # Erange_dptb = np.load('/data/zjj/NanoTCAD_interface/test_dptb_dev/dptb_hBN/dptb_negf_armchair_output/results/energy_range_k[0 0 0].npy')
        while E<=Eupper+0.5*dE: # be consistent with NanoTCAD ViDES setting
        # for E in Erange_dptb:
            self.lead_L.self_energy(kpoint=self.kpoints, energy=E, eta_lead=eta_lead)
            self.lead_R.self_energy(kpoint=self.kpoints, energy=E, eta_lead=eta_lead)
            self_energy_L = self.lead_L.se.numpy()
            self_energy_R = self.lead_R.se.numpy()
            self_energy_L_list.append(self_energy_L)
            self_energy_R_list.append(self_energy_R)
            Erange.append(E)
            E += dE

        np.save(self.results_path+'Erange.npy',np.array(Erange))
        np.save(self.results_path+'self_energy_L.npy',np.array(self_energy_L_list))
        np.save(self.results_path+'self_energy_R.npy',np.array(self_energy_R_list))       
